...
"""
The 'top_window_angle_spectrum_creation' function creates the top window which shows the angle spectrum visualisation.
...
and the audio data ('audio_data' variable) are determined. After that the Angle Spectrum is plotted, using the 
'angle_spectrum' function and, as arguments, the audio data and sampling frequency. To the plot are added title and 
labels (to x and y dimensions). The 'canvas' variable is used to create a canvas, that is used to embedd the plot to the 
Top Window, which is then packed, based on the Top Window dimensions. The 'toolbar' variable is create a native 
...
def top_window_angle_spectrum_creation():
    ...
    if not file_to_visualise_path:
        ...
        top_window.title("Angles Spectrum Visualization")
        ...
        ax.angle_spectrum(audio_data, Fs=fs)
        ax.set_title("Angles Spectrum")

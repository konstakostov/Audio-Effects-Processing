...
"""
The 'top_window_phase_spectrum_creation' function creates the top window which shows the phase spectrum visualisation.
...
After that the Phase Spectrum is plotted, using the 'phase_spectrum' function and, as arguments, the audio data and 
sampling frequency. To the plot are added title and labels (to x and y dimensions). The 'canvas' variable is used to 
create a canvas, that is used to embedd the plot to the Top Window, which is then packed, based on the Top Window 
dimensions. The 'toolbar' variable is create a native matplotlib toolbar and add it to Top Window, which provides 
...
def top_window_phase_spectrum_creation():
    ...
    if not file_to_visualise_path:
        ...
        top_window.title("Phase Spectrum Visualization")

        fs, audio_data = wavfile.read(file_to_visualise_path)
        audio_data = audio_data[:, 0]

        fig, ax = plt.subplots()
        ax.phase_spectrum(audio_data, Fs=fs)

...
"""
The 'top_window_magnitude_db_spectrum_creation' function creates the top window which shows the magnitude spectrum in dB
visualisation.
...
After that the Magnitude Spectrum, in dB, is plotted, using the 'magnitude_spectrum' function and, as arguments, the 
audio data and sampling frequency. To the plot are added title and labels (to x and y dimensions). The 'canvas' variable 
is used to create a canvas, that is used to embedd the plot to the Top Window, which is then packed, based on the Top 
...
def top_window_magnitude_db_spectrum_creation():
    ...
    if not file_to_visualise_path:
        ...
        top_window.title("Magnitude [Energy] Spectrum Visualization")

        fs, audio_data = wavfile.read(file_to_visualise_path)
        audio_data = audio_data[:, 0]

        fig, ax = plt.subplots()
        ax.magnitude_spectrum(audio_data, Fs=fs)
        ax.set_title("Magnitude [Energy] Spectrum Graph")
